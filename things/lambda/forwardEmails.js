...
var defaultConfig = {
  fromEmail: "dev@hotvenueapp.com",
  emailBucket: "hotvenue-email",
  emailKeyPrefix: "all/",
  subjectAddendum: " ( !! automatic forward !! )",
  forwardMapping: {
    "nicola@hotvenueapp.com": [
      "nicola@ballarini.me"
    ],
    "niccolo@hotvenueapp.com": [
    ...
    "info@hotvenueapp.com": [
      "nicola@ballarini.me"
    ],
    "team@hotvenueapp.com": [
      "nicola@ballarini.me"
    ],
    "@hotvenueapp.com": [
      "niccolo@olivieriachille.com"
...
exports.processMessage = function(data, next) {
  ...
  header = header.replace(/^From: (.*)/mg, function(match, from) {
    var fromText;
  ...
  // Add something in the subject to show it's an automatic forward
  header = header.replace(/^Subject: (.*)/mg, function(match, subject) {
    var subjectText = 'Subject: ' + subject.trim();

    if (data.config.subjectAddendum) {
      subjectText += data.config.subjectAddendum;
    }
...
exports.sendMessage = function(data, next) {
  ...
  data.log({
    level: "info",
    message: "sendMessage: Sending email via SES. " + "Original recipients: " + data.originalRecipients.join(", ") + ". Transformed recipients: " + data.recipients.join(", ") + "."
  });
